...
2. Отображение (View) – описание разметки пользовательского интерфейса + сопутствующий класс. View описывает как должен выглядеть пользовательский интерфейс и на какие события следует реагировать приложению
3. Связующий слой (ViewModel) – классов (чаще, совокупность классов), который отвечает за выполнение некоторых бизнес-операций, включая доступ к базе данных и взаимодействие с аппаратными узлами

В MVVM связующий слой ViewModel часто является контекстом данных (**DataContext**) для View. Это означает, что когда во View описывается связывание (binding), то используемые поля и методы будут вызываться у экземпляра класса ViewModel. Иногда DataContext устанавливается во View в инициализатора, а иногда, как статический ресурс.

Пример создания экземпляра ViewModel в XAML-коде View. Заметим, что vm - это синоним имени класса ViewModel в соответствующем пространстве имён:
...
Привязка ViewModel может быть выполнена и в коде, хотя для подобного решения требуется дополнительная мотивания, например, применение шаблона проектирования **Dependency Injection**:
...
public MainWindow()
{
	...
	DataContext = new BanknotesViewModel(new DialogCSV(), new CsvFileService());
}
```

К ViewModel могут относится не только класс являющийся контекстом данных, но дополнительные классы:

1. Обработчики команд, которые оператор может подать через View
2. Helper-ы, классы обеспечивающие выполнение запросов к базе данных, а также доступа к различным устройствам
