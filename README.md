...
Начальное приложение следует сгенерировать, используя генератор кода по шаблонам из состава Microsoft Visual Studio. Параметры шаблона: XAML, Windows, Desktop. В качестве Runtime рекомендуется использовать последнюю из доступных версий .NET (на май 2022 это .NET 6.0).

При разработке View, рекомендуется группировать связанные сущности (_cohesion_) в UserControls. Анти-паттерном является размещение всей верстки в одном .xaml-файле. Кажется разумным создать отдельную папку «Views», в которой размещать разработанные UserControl-ы. 

Создавать UserControl рекомендуется посредством генератора компонентов приложения из состава Visual Studio, см. [статью](https://www.wpf-tutorial.com/usercontrols-and-customcontrols/creating-using-a-usercontrol/).
